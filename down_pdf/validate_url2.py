...
#放网址的文件名
file = open('url/url_shenzhen.txt')
...
for b in bb:
    ...
    if 'html' not in tempUrl:
        try:
            opener.open(tempUrl)
            urltxt1 = open("output_url2/ok_list.txt", "a")
            urltxt1.write(tempUrl + "\n")
            urltxt1.flush()
            urltxt1.close()
            count1 = count1 + 1
        except:
            f = open("log2/error.log", 'a')
            f.write(tempUrl + "\n")
            traceback.print_exc(file=f)
            f.flush()
            f.close()
            urltxt2 = open("output_url2/error_list.txt", "a")
            urltxt2.write(tempUrl + "\n")
            urltxt2.flush()
            urltxt2.close()
            count2 = count2 + 1
    else:
        urltxt22 = open("output_url2/html_list.txt", "a")
